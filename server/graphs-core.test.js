...
import {
  NODE_TYPES, compat, dockCandidates, hasRejectedDock, settleEdges,
  edgeValid, edgeRule, stoneHeight, placementFree, validateDoc,
  portCapacity, portLoad, setPortRows, pinsOf,
} from './graphs-core.js';

const doc = (nodes, edges = []) => ({ v: 1, name: 't', nodes, edges, notes: [] });

describe('registry & geometry', () => {
  test('stone height = header + connector rows, repeat rows grow it', () => {
    assert.equal(stoneHeight('button'), 2);
    assert.equal(stoneHeight('timer'), 3);
    assert.equal(stoneHeight('range'), 3);
    const grown = { type: 'action', rep: { 'in:trigger': 2 } };
    assert.equal(stoneHeight(grown), 4);
  });

  test('pinsOf emits one instance per repeat row with consecutive rows', () => {
    const n = { id: 1, type: 'action', col: 1, row: 1, rep: { 'in:trigger': 1 } };
    const pins = pinsOf(n).filter(p => p.id === 'trigger');
    assert.equal(pins.length, 2);
    assert.deepEqual(pins.map(p => p.row), [3, 4].map(r => r - 1));  // rows 2 and 3
  });
...
describe('candidate search (DOCK-03)', () => {
  test('finds an exact-row direct candidate first', () => {
    ...
    assert.deepEqual(c[0].edge, [1, 'pressed', 2, 'start']);
...
describe('edge settling (DOCK-05/07, INV-01)', () => {
  test('moving a stone next to a partner docks it; moving away undocks', () => {
    const d = doc([
      { id: 1, type: 'button', col: 1, row: 1 },
      { id: 2, type: 'wait', col: 4, row: 1 },
    ]);
    assert.deepEqual(settleEdges(d, 2), { dropped: 0, added: 0 });
    d.nodes[1].col = 2;
    const r = settleEdges(d, 2);
    assert.equal(r.added, 1);
    assert.deepEqual(d.edges, [[1, 'pressed', 2, 'start']]);
    assert.equal(edgeRule(d, d.edges[0]), 'direct');
    d.nodes[1].col = 4;
    assert.equal(settleEdges(d, 2).dropped, 1);
    assert.deepEqual(d.edges, []);
  });

  test('a joint edge validates and reports its rule', () => {
    const d = doc([
      { id: 1, type: 'timer', col: 1, row: 1 },  // elapsed out at row 2
      { id: 2, type: 'gate', col: 2, row: 0 },   // open in at row 2
    ], [[1, 'elapsed', 2, 'open']]);
    assert.equal(edgeValid(d, d.edges[0]), true);
    assert.equal(edgeRule(d, d.edges[0]), 'pulse');
  });
});
...
describe('multi rows (MULTI-01/02)', () => {
  test('capacity gates docking; growing a row unlocks a second source', () => {
    ...
    assert.equal(settleEdges(d, 2).added, 0);      // capacity 1 already used
    assert.equal(setPortRows(d, d.nodes[2], 'in', 'trigger', +1), true);
    assert.equal(portCapacity(d.nodes[2], 'in', 'trigger'), 2);
    const r = settleEdges(d, 2);                   // trigger row 3 now exists
    assert.equal(r.added, 1);
...
describe('validateDoc (VAL-02)', () => {
  test('accepts a consistent document and flags overlap + undockable edges', () => {
    const good = doc([
      { id: 1, type: 'button', col: 1, row: 1 },
  ...
  test('flags edges beyond a port\'s row capacity', () => {
    const d = doc([
      { id: 1, type: 'button', col: 1, row: 1 },
  ...
  test('every registry entry produces a positive height', () => {
    for (const t of Object.keys(NODE_TYPES)) assert.ok(stoneHeight(t) >= 2, t);
  });
});
