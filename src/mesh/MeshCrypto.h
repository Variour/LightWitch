...
inline bool encrypt(const uint8_t key[AES_KEY_LEN], const uint8_t* plain, size_t len,
    ...
    bool ok = mbedtls_gcm_setkey(&gcm, MBEDTLS_CIPHER_ID_AES, key, AES_KEY_LEN * 8) == 0 &&
              mbedtls_gcm_crypt_and_tag(&gcm, MBEDTLS_GCM_ENCRYPT, len, nonce, NONCE_LEN,
                                        nullptr, 0, plain, cipher, TAG_LEN, tag) == 0;
...
inline bool decrypt(const uint8_t key[AES_KEY_LEN], const uint8_t* in, size_t len,
    ...
    mbedtls_gcm_init(&gcm);
    bool ok = mbedtls_gcm_setkey(&gcm, MBEDTLS_CIPHER_ID_AES, key, AES_KEY_LEN * 8) == 0 &&
              mbedtls_gcm_auth_decrypt(&gcm, cipherLen, nonce, NONCE_LEN, nullptr, 0,
                                       tag, TAG_LEN, cipher, outPlain) == 0;
    mbedtls_gcm_free(&gcm);
