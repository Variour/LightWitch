...
Behaviour is built in a visual editor by docking blocks together. Anything a device can notice gets wired to anything a device can do, then distributed to every device in the mesh with matching hardware. No code required.

Nothing says a device may only have one job. A row of bed lamps lights the path and watches the soil at the same time, and starts the pump when things get too dry. Actually smart stuff, everywhere you feel like putting it.
...
**Supported hardware:** only the [Waveshare ESP32-S3-AUDIO-Board](docs/boards/waveshare-esp32-s3-audio.md) has an SD card reader — its firmware detects the card automatically at boot, with nothing to configure in Settings. Other boards (plain ESP32, ESP32-C3, generic ESP32-S3 devkits) don't have this reader and won't show a Storage section at all.
...
3. Create a GitHub **fine-grained personal access token** with the following permissions:
   ...
   - **Repository permissions:**
     ...
     - `Pull requests` — **Read-only** (needed for **Allow installing PR builds**)
